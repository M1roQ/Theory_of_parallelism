...
class Global:
    ...
    # Значения по умолчанию — будут перезаписаны в benchmark
    num_workers = 1
    buffer_size = 2
...
class WindowImage:
    def __init__(self, resolution: tuple[int, int], show: bool = True, save: bool = False, out_file: str = "results/output_video.mp4"):
        self._resolution = resolution
        self._save = save
        self._show = show
        if save:
    ...
    def __del__(self):
        if self._save and self.out is not None:
            self.out.release()
...
def single_threaded_process(cam: SensorCam, detector: Detector, window: WindowImage) -> float:
    start = time.time()
    frame_id = 0
    ...
    while True:
        ...
        if frame is None:
            break
        _, frame = detector.predict(frame_id, frame)
        window.show(frame)
        frame_id += 1
    end = time.time()
    total_time = end - start
    print(f"Total time (Single-threaded): {total_time:.5f} sec")
    return total_time


def multi_threaded_process(cam: SensorCam, window: WindowImage, num_workers: int) -> float:
    Global.num_workers = num_workers
    Global.buffer_size = 2
    Global.input_frame_buffer = queue.Queue(maxsize=Global.buffer_size)
    Global.output_frame_buffer = {}
    Global.stop_flag.clear()
    Global.finish_video.clear()
    Global.next_id = 0
    ...
    while True:
        if Global.finish_video.is_set() and Global.input_frame_buffer.empty():
            break
        with Global.output_frame_buffer_lock:
            if last_id in Global.output_frame_buffer:
    ...
    total_time = end - start
    print(f"Total time ({num_workers} threads): {total_time:.5f} sec")
    return total_time


def benchmark(input_file: str, resolution: tuple[int, int], output_file: str):
    thread_counts = [1, 2, 3, 4, 5, 6, 7, 8, 9, 12]

    # Однопоточный режим — создаём камеру и окно один раз
    cam = SensorCam(input_file, resolution)
    window = WindowImage(resolution, show=False, save=False)

    detector = Detector()
    single_time = single_threaded_process(cam, detector, window)

    cam.cap.release()  # Закрываем камеру

    with open(output_file, "w") as f:
        f.write(f"Single-threaded: {single_time:.5f}\n")
        f.write(" ".join(str(t) for t in thread_counts) + "\n")

        times = []
        for tcount in thread_counts:
            # Создаём новую камеру каждый раз, чтобы начать сначала
            cam = SensorCam(input_file, resolution)
            window = WindowImage(resolution, show=False, save=False)

            t = multi_threaded_process(cam, window, tcount)
            cam.cap.release()
            times.append(t)

        # Записываем времена одной строкой
        f.write(" ".join(f"{t:.5f}" for t in times) + "\n")

        # Вычисляем и записываем ускорения второй строкой
        accelerations = [single_time / t for t in times]
        f.write(" ".join(f"{s:.5f}" for s in accelerations) + "\n")

    print("Benchmark finished. Results saved to", output_file)




def main(input_file: str, output_file: str, resolution: str, mode: str) -> None:
    ...
    logging.basicConfig(
        ...
        filename="log/logs.log", filemode="a"
    )
    ...
    cam = SensorCam(input_file, resolution)
    window = WindowImage(resolution, show=True, save=output_file is not None, out_file=output_file)

    if mode == "single":
        detector = Detector()
        single_threaded_process(cam, detector, window)
    elif mode == "benchmark":
        benchmark(input_file, resolution, output_file or "benchmark_results.txt")
    else:
        raise ValueError("Unknown mode. Use 'single' or 'benchmark'.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-i", "--input_file", type=str, default="video.mp4", help="Path to video file or camera device.")
    parser.add_argument("-o", "--output_file", type=str, default=None, help="Output video file name.")
    parser.add_argument("-r", "--resolution", type=str, default="640x480", help="Resolution format WxH.")
    parser.add_argument("-m", "--mode", type=str, choices=["single", "benchmark"], default="single", help="Execution mode: single or benchmark.")

    args = parser.parse_args()

    try:
        main(args.input_file, args.output_file, args.resolution, args.mode)
    except Exception as exp:
        logging.error(f"Some errors occurred: {exp}")
